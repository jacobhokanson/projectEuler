...
def LargestinSeries(n):
    first, last = 0, n
    greatest = 0
    while not last == len(digits):
        current = reduce(lambda f, n: f * n, digits[first:last])
        if current > greatest:
            # print(f'greatest is {current} at indexes [{first}:{last}]')
            greatest = current
